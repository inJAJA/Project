...
from utils.utils import preprocess, invert_affine, postprocess, STANDARD_COLORS, standard_to_bgr, \
...
video_path = os.path.join('datasets', project, 'sample_video', 'son.mp4')  # video load

# img_name
if type(video_path) == list:
    img_names = [i.split('/')[-1] for i in video_path]
else:
    img_names = [video_path.split('/')[-1]]
