...
from efficientdet.vocdataset import VocDataset, Resizer, Normalizer, Augmenter, collater
...
def get_args():
    ...
    parser.add_argument('-p', '--project', type=str, default='VOC2012', help='project file that contains parameters')
...
def train(opt):
    ...
    training_set = VocDataset(root_dir=os.path.join(opt.data_path, params.project_name), set=params.train_set, params = params,
                               transform=transforms.Compose([Normalizer(mean=params.mean, std=params.std),
                                                             Augmenter(),
                                                             Resizer(input_sizes[opt.compound_coef])]))
    training_generator = DataLoader(training_set, **training_params)

    val_set = VocDataset(root_dir=os.path.join(opt.data_path, params.project_name), set=params.val_set, params = params,
                          transform=transforms.Compose([Normalizer(mean=params.mean, std=params.std),
                                                        Resizer(input_sizes[opt.compound_coef])]))
